...
const registerUser = asyncHandler( async (req, res) => {
    ...
    if (
        [fullName, email, username, password, team_role, teams].some((field) => field?.trim() === "")  
    ) {
    ...
    const createdUser = await User.findById(user._id).select(
        "-password -refreshToken" //.select se ye sab nhi aaegaa
    )
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    const incomingRefreshToken = req.cookies.refreshToken || req.body.refreshToken //seasion or cookie dono same hn wohi se token mil jaegaa 
